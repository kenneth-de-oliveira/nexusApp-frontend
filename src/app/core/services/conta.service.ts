...
import { BoletoStatusDTO } from "../dtos/boleto-status.dto";
...
export class ContaService {
  ...
  getExtratoPdfConta(id: number): Observable<any> {
    return this.apiService.getArquivo(`${this.controller}/${id}/extratos/pdf`);
  }

  getCodigoDeBarras(codigoDeBarras: string): Observable<any> {
    return this.apiService.get(`${this.controller}/boletos/${codigoDeBarras}`);
  }

  updateBoletoStatus(boletoStatusDTO : BoletoStatusDTO) : Observable<any>{
    return this.apiService.patch(`${this.controller}/boletos/update`, boletoStatusDTO);
  }
